...
public class scr_SmartestZombie : MonoBehaviour {
    ...
    public bool canFly = true; //Flying alters float behavior to ignore gravity. The enemy will fly up or down only to sustain floatHeight level.
    ...
    public float runDistance = 10.0f; //If the enemy should keep its distance, or charge in, at what point should they begin to run?
    ...
    public int walkSpeed = 1; //Standard movement speed.
    public int runSpeed = 3; //Movement speed if it needs to run.
    public int randomSpeed = 2; //Movement speed if the AI is moving in random directions.
    public float rotationSpeed = 3.0f; //Rotation during movement modifier. If AI starts spinning at random, increase this value. (First check to make sure it's not due to visual radius limitations)
    public float visualRadius = 10.0f; //How close does the player need to be to be seen by the enemy? Set to 0 to remove this limitation.
    public float moveableRadius = 50.0f; //If the player is too far away, the AI will auto-matically shut down. Set to 0 to remove this limitation.
    public float attackRange = 1.0f; //How close does the enemy need to be in order to attack?
    ...
    public float huntingTimer = 1.0f; //Search for player timer in seconds. Minimum of 0.1

    public bool estimateElevation = true; //This implements a pause between raycasts for heights and guestimates the need to move up/down in height based on the previous raycast.
