...
public class src_RealZombie : MonoBehaviour {
	...
	void blueSet () {
		blue = true;
		red = false;
		//Debug.LogError("blueSet");
	}
	void redSet () {
		blue = false;
		red = true;
		//Debug.LogError("redSet");
	}
	void shot () {
		if (health == 1) {
			Destroy(gameObject);
...
//		Debug.LogError(y);
		...
		if (currentPosition.x < lastKnownPosition.x && currentPosition.y < lastKnownPosition.y && blue ) {
			x = 0.25f;
			y = -0.25f;
		} else if (currentPosition.x < lastKnownPosition.x && currentPosition.y < lastKnownPosition.y && red ) {
			x = -0.25f;
			y = 0.25f;
		} else if (currentPosition.x > lastKnownPosition.x && currentPosition.y < lastKnownPosition.y && blue ) {
			x = -0.25f;
			y = -0.25f;
		} else if (currentPosition.x > lastKnownPosition.x && currentPosition.y < lastKnownPosition.y && red ) {
			x = 0.25f;
	...
	void OnTriggerStay(Collider other) {
		...
		if(other.gameObject.tag == "Player") {
			...
			transform.position -= transform.forward * runSpeed * Time.deltaTime * 10;
